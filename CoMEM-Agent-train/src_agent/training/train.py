...
from qwenVL_compressor import Qwen2_5_VLForConditionalGeneration_new
from qwenVL_compressor2 import Qwen2VLForConditionalGeneration_new
# import wandb
...
def configure_llm(model, training_args):
    ...
    knowledge_processor_params = model.knowledge_processor.parameters()
    set_requires_grad(knowledge_processor_params, True)
    
    inference_params = model.model_inf.parameters()
    set_requires_grad(inference_params, False)
    # set_requires_grad(inference_params, True)
...
def train():
    ...
    if "Qwen2.5" in model_args.model_id:
        # EDIT: rewrite the model loading to use the new class
    ...
    # EDIT: freeze the model_inf parameters
    for name, param in model.model_inf.named_parameters():
        param.requires_grad = False
        # param.requires_grad = True
    for name, param in model.knowledge_processor.named_parameters():
        param.requires_grad = True
    ...
    if training_args.lora_enable:
        # EDIT: filter out modules for model_inf and knowledge_processor, only add LoRA to the compress model
        lora_namespan_exclude = training_args.lora_namespan_exclude
        all_modules = find_target_linear_names(model, lora_namespan_exclude=lora_namespan_exclude, num_lora_modules=training_args.num_lora_modules, verbose=False)
        filtered_modules = [m for m in all_modules if not ('model_inf' in m) and not ('knowledge_processor' in m)]
        print(f"Filtered LoRA modules: {filtered_modules}")
        peft_config = LoraConfig(
            r=training_args.lora_rank,
            lora_alpha=training_args.lora_alpha,
            target_modules=filtered_modules,
            modules_to_save=["knowledge_processor"],
    ...
    qformer_params = sum(p.numel() for p in model.knowledge_processor.parameters() if p.requires_grad)
    print(f"Training {qformer_params} parameters in the Q-Former")
